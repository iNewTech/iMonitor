...
`public/shared/support.js` owns the online User Guide URL and reuses the existing Support action lifecycle to open it through `openExternalUrl`. Connect, ActionBoard, and Settings each expose the same `#support-user-guide` menu button. Keep the URL on the public branch containing the current guide (`dev` today); no guide copy, new IPC endpoint, authentication gate, or renderer navigation is needed. The shared action handles pending/failed opens and preserves contact and diagnostics behavior (#63).
