...
`src/features/ibmeyeai/grounded-guidance.ts` builds the selected-job AI evidence boundary. It labels observed facts, evidence references, interpretation, missing sources, suggested checks, and matching approved runbooks separately. It bounds and redacts provider output, checks the required response sections, removes instruction-like evidence text, and preserves stable references. AI output remains advisory and cannot become an IBM i command.
...
`src/features/action-board/action-planner.ts` provides the common proposal shape used by selected-job details, job-scoped AI, runbook guidance, approved Resolution Memory, and the MCP action catalog. Its primary recommendation is lifecycle-aware: claim unassigned work, then prefer a ready runbook/operator action, then advisory evidence, then escalation. Proposals include their source, effect, risk, permission, evidence, verification, and citation fields. The planner is descriptive only; the main process still requires explicit authorization, preview, approval, execution, and independent verification for any write.
...
`src/features/action-board/incident-replay.ts` owns the ten versioned training scenarios and the deterministic playback result. `src/main/ipc/incident-replay-ipc.ts` exposes only catalog and in-memory replay operations behind read authorization; it receives no IBM i service, command runner, or external delivery runtime. The task window labels every result as training-only and records no production mutation.

`src/features/action-board/resolution-memory.ts` is the customer-scoped knowledge boundary. It captures verified incident outcomes as drafts with operator and source-incident lineage, retains review history, and requires explicit approval before retrieval can use them. Reviewers can revise, reject, retire, or export entries; revisions supersede the previous version until re-approved. Matching reports confidence, freshness, environment compatibility, and changed runtime fingerprints, while default retrieval excludes stale, conflicting, incompatible, and unreviewed entries. The store is local to the customer and is never a direct command source for IBM i actions.

`src/features/history/support-metrics.ts` derives support outcome measurements from the customer-scoped incident ledger and system-scoped AI request observations. It uses half-open timestamp windows, separate measured/unknown counts, and prior equal-window comparisons. An unresolved or reopened current cycle pauses recovery timing until a later verified clear; a missing timestamp is never replaced with an estimate. `support-metrics-ipc.ts` applies read authorization, filters to the connected system, and owns JSON export. The panel is read-only and records autonomous recovery as zero because production corrections still require an operator.
