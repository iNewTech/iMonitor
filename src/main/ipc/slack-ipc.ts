...
export function registerSlackIpc(dependencies: RegisterSlackIpcDependencies) {
    ...
    ipcMain.handle('save-slack-settings', (_event, settings) => { dependencies.requirePremium(); return dependencies.saveSlackSettings(settings); });
    ipcMain.handle('send-test-slack-message', async () => { dependencies.requirePremium(); return dependencies.sendTestSlackMessage(); });
}
